...
if (!$inspection || $inspection['company_id'] !== getActiveCompanyId()) {
    ...
    echo json_encode(['error' => 'You do not have permission to delete this option.']);
    exit;
}
...
if ($inspection['status'] !== 'draft') {
    http_response_code(403); // Forbidden
    echo json_encode(['error' => 'Cannot delete options from a published inspection. Please unpublish it first.']);
    exit;
}

// Delete the option
if (!deleteQuestionOption($optionId)) {
    http_response_code(500); // Internal Server Error
    echo json_encode(['error' => 'Failed to delete option. Please try again.']);
    exit;
}
